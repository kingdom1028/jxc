 ...
 * 最后修正：2010-6-29
 **/
var IMAGESPATH = '/Content/zDialog/images/'; //图片路径配置，如在有iframe情况下使用，此路径为最顶层窗口页面的相对路径
...
var Dialog = function () {
    ...
    this.okButton = null;
    this.okButtonIsShowed = true;//是否显示确定按钮 add 李长好 by 2012-09-28
...
Dialog.prototype.create = function () {
  ...
  <table id="_DialogTable_{thisID}" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: -moz-none; font-size:12px; line-height:1.4;border-collapse: collapse;">\
    ...
    if (this.okButtonIsShowed)//add lichanghao by 2012-09-28
    {
        html += '<input type="button" style="background:url({IMAGESPATH}dialog_btn.gif) no-repeat 0 50%; border:0px;font-size:12px; font-weight:bold; color:#fff; width:56px; height:23px; line-height:23px;cursor: pointer;" value="' + this.OkButtonText + '" id="_ButtonOK_{thisID}"/>&nbsp;&nbsp;';
        html += '&nbsp;&nbsp;<input type="button" style="background:url({IMAGESPATH}dialog_btn.gif) no-repeat 0 50%; border:0px;font-size:12px; font-weight:bold; color:#fff; width:56px; height:23px; line-height:23px;cursor: pointer;" value="' + this.CancelButtonText + '" onclick="Dialog.getInstance(\'{thisID}\').close();" id="_ButtonCancel_{thisID}"/>';
    }
    else
    {
        html += '&nbsp;&nbsp;<input type="button" style="background:url({IMAGESPATH}dialog_btn.gif) no-repeat 0 50%; border:0px;font-size:12px; font-weight:bold; color:#fff; width:56px; height:23px; line-height:23px;cursor: pointer;" value="' + this.OkButtonText + '" onclick="Dialog.getInstance(\'{thisID}\').close();" id="_ButtonCancel_{thisID}"/>&nbsp;&nbsp;';
    }
      html+='</div></td>\
          </tr>\
        </table></td>\
...
Dialog.prototype.addButton = function (id, txt, func) {
    ...
    button.type = "button";
    button.style.cssText = ("margin-right:15px; background:url({IMAGESPATH}dialog_btn2.gif) no-repeat 0 50%; border:0px;font-size:12px; font-weight:bold; color:#fff; width:80px; height:23px; line-height:23px;cursor: pointer;").replace(/\{IMAGESPATH\}/gm, IMAGESPATH).replace(/\{thisID\}/gm, this.ID);
    button.value = txt;
    button.onclick = func;
...
Dialog.alert = function (msg, func, w, h) {
    parent.$.messager.alert('系统提示', msg);
    return;
    var w = w || 300,
        h = h || 110;
    var diag = new Dialog({
    ...
    diag.okButtonIsShowed = false;
    diag.Title = "系统提示";
    diag.CancelEvent = function () {
        diag.close();
        if (func) func();
    };
    diag.InnerHtml = '<table height="98%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="center" width="50"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_alert.gif" width="34" height="34" align="absmiddle"></td>\
			<td align="left" id="Message_' + this.ID + '" style="font-size:9pt">' + msg + '</td></tr>\
	</table>';
    diag.show();
    //diag.cancelButton.parentNode.style.textAlign = "center";
    //diag.okButton.style.display = "none";
    //diag.cancelButton.value = diag.OkButtonText;
    //diag.cancelButton.focus();
};
Dialog.confirm = function (msg, funcOK, funcCal, w, h,OKText,CancelText) {
    var w = w || 300,
        h = h || 110;
    ...
    diag.OkButtonText = OKText;
    diag.CancelButtonText = CancelText;
    diag.ShowButtonRow = true;
    diag.okButtonIsShowed = true;
    diag.Title = "信息确认";
    diag.CancelEvent = function () {
        diag.close();
        if (funcCal) funcCal();
    ...
    diag.InnerHtml = '<table height="98%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="center" width="50"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_query.gif" width="34" height="34" align="absmiddle"></td>\
			<td align="left" id="Message_' + this.ID + '" style="font-size:9pt">' + msg + '</td></tr>\
	</table>';
    diag.show();
